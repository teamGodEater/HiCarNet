...
public class WeiZhangDetialFragment extends BaseFragment {
    ...
    protected WindowsParams onCreateSupportViewParams(WindowsParams windowsParams) {
        windowsParams.rootLayoutId = R.layout.frgm_weizhang_detail;
    ...
    private void setRv() {
        ...
        for (WeizhangResponseHistoryJson bean : historys) {
            count++;
            View view = LayoutInflater.from(mRootContain.getContext()).inflate(R.layout.item_weizhang, mRootContain, false);
    ...
    private void finish() {
        if (hasFinish)
            return;
        hasFinish = true;
        mTopView.animate().translationX(mTopView.getWidth())
